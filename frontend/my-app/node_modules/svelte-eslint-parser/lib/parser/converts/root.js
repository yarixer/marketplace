    ...
    if (comments && Array.isArray(comments) && comments.length > 0) {
        ...
        while ((element = elementsBuffer.shift())) {
            ...
            if (element.type === "SvelteElement") {
                elementsBuffer.unshift(...element.children.filter((node) => node.type === "SvelteElement"));
            }
        }
        // Process all comments from Svelte compiler
        for (const comment of comments) {
            if (comment.type !== "Block" && comment.type !== "Line") {
                throw new ParseError(`Unknown comment type: ${comment.type}`, comment.start, ctx);
            }
            const tag = tags.find((tag) => tag.range[0] <= comment.start && comment.end <= tag.range[1]);
            if (!tag) {
                // If the comment is outside of all tags, it will be skipped.
                continue;
            }
            if (tag.attributes.some((attr) => attr.range[0] <= comment.start && comment.end <= attr.range[1])) {
